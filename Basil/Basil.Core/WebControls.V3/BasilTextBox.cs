...
namespace Basil.WebControls.V3
{
    public class BasilTextBox : TextBox, IBasilWebControl
    {
        ...
        protected override void Render(HtmlTextWriter writer)
        {
            ...
            if (!IsValid) cssClass += string.Format(" {0}{1}", Validator.Settings.Textbox.ErrorCssClass, feedback);
            if (IsWarning) cssClass += string.Format(" {0}{1}", Validator.Settings.Textbox.WarningCssClass, feedback);
            if (IsInfo) cssClass += string.Format(" {0}{1}", Validator.Settings.Textbox.InfoCssClass, feedback);
            if (IsSuccess) cssClass += string.Format(" {0}{1}", Validator.Settings.Textbox.SuccessCssClass, feedback);
